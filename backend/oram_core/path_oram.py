...
class PathORAM:
    ...
    def _access(self, idx: int, op: str, new_val, override_leaf=None):
        ...
        max_allowed = self.Z * (self.L + 2)  # Allow some extra stash space to account for recursion and path overlap
        if len(self.stash) > max_allowed:
            print(f"⚠️ WARNING: Stash overflow — current size: {len(self.stash)} (threshold was {max_allowed})")
